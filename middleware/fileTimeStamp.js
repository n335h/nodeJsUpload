...
const fileTimeStamp = async (req, res, next) => {
    try {
        const filepath = path.join(__dirname, '../files', 'files.json');
        ...
        await fs.writeFile(filepath, JSON.stringify(filesJson));

        next();
    } catch (error) {
        // Handle errors appropriately
        console.error('Error in fileTimeStamp middleware:', error);
